...
happyExpList = Happy_Data_Array.listArray (0,23) ([704,16392,0,576,0,11264,1024,0,9217,0,36,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parser","expr","ap","atom","ident","lambda","'.'","'('","')'","%eof"]
...
happyReduction_6 (HappyTerminal (L.Identifier happy_var_1))
	 =  HappyAbsSyn6
		 (Var happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	L.Identifier happy_dollar_dollar -> cont 7;
	L.Lambda -> cont 8;
	L.Dot -> cont 9;
	L.OpenParen -> cont 10;
...
happyError' :: () => ([(L.Token)], [Prelude.String]) -> Either ParseError a
...
instance RenderError ParseError where
  render EmptyTokenStream = RenderedError "ParseError: Empty token stream."
  render (UnexpectedToken tok) = RenderedError $ "ParseError: Unexpected token '" <> show tok <> "'."

data RenderedError = RenderedError { message :: String }
  deriving Show
