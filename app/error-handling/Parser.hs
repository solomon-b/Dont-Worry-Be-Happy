...
happyReduction_1 ((HappyAbsSyn5  happy_var_4) `HappyStk`
	_ `HappyStk`
	(HappyTerminal (L.TokenExt (L.Identifier happy_var_2) _)) `HappyStk`
	_ `HappyStk`
	happyRest)
	 = HappyAbsSyn4
...
happyReduction_6 (HappyTerminal (L.TokenExt (L.Identifier happy_var_1) _))
	 =  HappyAbsSyn6
		 (Var happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	L.TokenExt (L.Identifier happy_dollar_dollar) _ -> cont 7;
	L.TokenExt L.Lambda _ -> cont 8;
	L.TokenExt L.Dot _ -> cont 9;
	L.TokenExt L.OpenParen _ -> cont 10;
	L.TokenExt L.CloseParen _ -> cont 11;
	_ -> happyError' ((tk:tks), [])
	}
